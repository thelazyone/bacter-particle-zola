import { wasm_memory, Petri, default as init } from './bacter_rust.js';

    async function run() {
      await init('./js/bacter_rust_bg.wasm');
      ...
      const renderLoop = () => {
        ...
        // simulation proceeds by one tick or by 1000.
          ...
          ctx.fill();
          for (let i = 0; i < bacters_number; i++) {
            drawCircle(ctx, positions[2*i], positions[2*i + 1], sizes[i] * 10., 'LightSlateGray', '', 2)
          }
